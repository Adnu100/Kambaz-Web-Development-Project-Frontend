...
export default function QuizDetails() {
  ...
  const [questionsCount, setQuestionsCount] = useState(0); // Add this state
  ...
  useEffect(() => {
    getQuiz();
    getQuizQuestions(); // Add this to fetch questions count
        ...
        <div className="wd-quiz-details-actions d-flex justify-content-center mb-4">
          {attempt._id === "none" && (
            <Button variant="primary" className="wd-btn-primary" onClick={async (e) => {
              ...
              router.push(`/Courses/${cid}/Quizzes/${qid}/attempt/${attemptId}`);
            }}>
              Attempt Quiz
            </Button>
          )}
          ...
          {attempt._id !== "none" && (
            <Button variant="secondary" className="wd-btn-primary me-2" onClick={async (e) => {
              e.preventDefault();
              const quizAttempt = await quizClient.findQuizAttemptById(cid, qid, currentUser._id);
              router.push(`/Courses/${cid}/Quizzes/${qid}/results/${quizAttempt[0]._id}`);
            }}>
              View Results
            </Button>
          )}
      ...
      <div>
        ...
        <div id="wd-details" className="ms-5 mt-3 mb-5 w-50">
          ...
          <div className="row mt-2">
            ...
            <div className="col-3">{quiz.timeLmt ? `${quiz.timeLmt} minutes` : "No limit"}</div>
          </div>
          ...
          <div className="row mt-2">
            ...
            <div className="col-3">{quiz.showCorrectAnswers ? "Yes" : "No"}</div>
          </div>
